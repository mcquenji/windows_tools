...
class RouteInfo {
  ...
  /// A function that generates the route title from the [context]
  ///
  /// Note: This is for internal use only. If you want to get the route title, use [title]
  final String Function(BuildContext t)? titleGenerator;
  ...
  /// Generates the route title.
  ///
  /// Do not use [titleGenerator]
  String title(BuildContext context) {
    return titleGenerator?.call(context) ?? parent?.titleGenerator?.call(context) ?? routeName.replaceAll('/', ' ').toTitleCase();
  }
  ...
  void validateParameters(Map<String, dynamic>? args) {
    if (this.args != null) {
      ...
      for (var paramDef in this.args!.entries) {
        var paramValue = args![paramDef.key];
        assert(paramValue != null, "Missing argument ${paramDef.key}");

        assert(paramValue.runtimeType == paramDef.value, "Argument ${paramDef.key} is of type ${paramValue.runtimeType} but should be of type ${paramDef.value}");
      }
    }
  }
